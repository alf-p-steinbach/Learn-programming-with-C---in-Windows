...
The three next examples illustrate the pure text environments known as *console windows*, which are much used, almost indispensable, in programming work. First there’s one example of using a command interpreter in a console window (most/all command interpreters run in console windows). Then come two minimal examples of programs to present a single line of text in a console, in (in order) C++ and C#, and this is the basis of the majority of beginner’s programs.
