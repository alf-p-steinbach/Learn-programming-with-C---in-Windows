...
Next a general window with flowed design is presented using C# and Windows Forms. This mainly illustrates that general window (GUI) stuff, as opposed to message boxes, is complex and involves much code. On a first reading you’re only expected to get a general feeling for what such code is like.

The three next examples illustrate the pure text environments known as *console windows*. First one example using a command interpreter in a console window, which is very common for programming work. Then, because this is the basis of the majority of beginner’s programs, come two minimal examples of programs to present a single line of text, in (in order) C++ and C#.

All the examples so far just present a line of text with the text specified in the code. As if a program can only present and generally do exactly what you have ordered via the instructions, like an exact echo. But a program can do much more, even totally unanticipated things, by repeating execution of parts of the code, called a *loop*, by changing the contents of memory, and by conditionally executing some code part or not depending on memory contents, called a *choice* or *selection*.

The eight’ example therefore illustrates an unexpected “more” effect, namely a so called Sierpinski triangle, resulting from a loop, change of memory contents, and some choices, in C++.

For comparison the ninth example does exactly the same in C#.

Then, to show how graphics can be much more engaging, more satisfying, than just console text, the Sierpinski triangle computed in the same way is presented in a general window using C#.

And to round up the examples the eleventh example shows how to do that with JavaScript in a browser.
