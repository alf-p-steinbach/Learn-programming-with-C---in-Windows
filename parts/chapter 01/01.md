...
You *can* feed JavaScript code *directly* to your browser, of old e.g. by typing `javascript:alert('Hello from JavaScript!')` in its address bar. But while this now still worked in my Microsoft Edge browser, in my Chrome it didn’t work for new tabs, only for tabs that already had some content, and in my Firefox it didn’t work at all. In all browsers one can however use the developer tools’ (keypress `Ctrl`+`Shift`+`I`) console (click somewhere that says “Console”) to do this.
