...
* *Interpretation:*  
  ...
  In ordinary everyday language an interpreter translates from one spoken language to another, or perhaps is one who explains something. In programming an interpreter “translates” from instructions to actions, effects.
...
* while some of the instructions you use map more or less directly to just a handful of built in computer instructions, i.e. result in the execution of at most a handful of machine code instructions, some of the instructions that you use map to at least 263 zillion machine code instructions (whatever).

Of course a modern computer executes its machine code at breakneck speed. The old PC that I’m writing this on has 6 CPU “cores” that each executes one instruction sequence. Each does 2.4 billion *steps* per second. Now, some instructions may take more than one step, but on the other hand in some cases several instructions may effectively be executed, in parallel, in a step. So if a high level instruction that you use, let’s say it requires and waits for the user to type in something, if that takes 7 seconds, then on this PC the 263 zillion machine code instructions could be 16.8 billion. Or whatever, these are huge, gargantuan numbers, which tells you that

* each machine code instruction *really* does very very little, and
...
The three last examples then illustrate the pure text environments known as *console windows*, which are much used, they’re almost indispensable!, in programming work. First there’s one example of using a command interpreter in a console window (most/all command interpreters run in console windows). Then come two minimal examples of programs to present a single line of text in a console, in C++ and in C#, and this is the basis of the majority of beginner’s programs.

These examples mainly show how code looks and how different environments and programming languages are, well, different.

So in a way all the examples here *do* the same: they present some given text. Just in different ways and in different environments. I judged that it would be too much with even more examples that e.g. would show how a program can do more than you put into it, such as displaying an impressive and surprising Mandelbrot set graphic. But the “do more” is covered later in the book.
...
A large part of Windows’ functionality is not directly accessible to standard C++ code because it’s part of [**.NET**](https://en.wikipedia.org/wiki/.NET "Wikipedia’s “.NET” article"), a set of instruction definitions (and other stuff) that builds on the core Windows functionality but that unlike the core Windows functionality isn’t designed for the C and C++ languages. .NET has been part of Windows since Windows Vista. .NET and standard C++ are a bit at odds not just because of the technical details but also because .NET is a “safe” **managed environment** where a program can’t just do whatever it wants, in particular with respect to memory management, while the point of C++ is to be able to do anything, including full control over the use of memory.
...
After using the requisite tools to create a corresponding executable file “text-hello.exe”, the base name of that executable file, “text-hello”, can be used as a command to run it:

<img src="images/sshot-text-hello.annotated.png" width="66%">

I.e. a new command `text-hello` was created, and that command was created in a way that is very simple when one just has the tools installed and some experience with using them.
...
Such console programs can be very useful as tools for programming work. In fact most tool that we’ll use, including the `csc` C# compiler that I used to compile this program, and including the mentioned `cl` and `g++` C++ compilers, are console programs. And creating console programs can be a very practical way to learn programming, because one avoids all the extra presentation- and interaction-related stuff and complexity of a GUI program, that you have seen.

But in addition console output from a GUI program can be useful for *creating* a GUI program, because it lets you see what’s going on on the inside.

There are also other ways to do that, in particular using a special tool called a debugger, but **trace output** to a console, simply placing instructions that produce informative output at critical points in the program, is a technique that’s always available and a technique where you have full control.
