...
- [2.1 &nbsp;“Hello, world!”.](#21-hello-world)
  ...
  - [2.1.2 &nbsp;Type or paste in the source code &mdash; correctly!](#212-type-or-paste-in-the-source-code-mdash-correctly)
...
- [2.2 &nbsp;Static HTML &rarr; event &rarr; dynamic JavaScript execution.](#22-static-html--event--dynamic-javascript-execution)
...
### 2.2 &nbsp;Static HTML &rarr; event &rarr; dynamic JavaScript execution.

You may rightly wonder what the difference is between using JavaScript to present a line of text, versus just writing that text line directly as HTML, which would seem far more practical?

Well, the JavaScript message box is a temporary thing, an action, a *dynamic* effect. It disappears when you click OK, generally then leaving the page’s contents as it was, except when the browser hasn’t yet finished presenting the page. In contrast page content specified as HTML is *static*, even though the browser may be slow about presenting it while it’s running JavaScript to present message boxes.

This difference is better observed with an example where the message box only pops up after the page content has been presented, e.g. when the user clicks a button. **Events** involving an HTML button, such as the event “the button was clicked”, can be *connected* to JavaScript by writing e.g. `onclick=`&hellip;, with the JavaScript code enclosed in HTML double quote characters. It is for this reason that by strong convention single quote characters are used in the JavaScript code itself:
...
You can of course click the button how many times you want, not just one time. Each time the JavaScript code is executed. Which causes the message box to pop up.

In passing: the button’s total grayness, its lack of color, is not all the way down at the default Windows message box level, but it doesn’t feel entirely right either. Coloring and visible reactions (feedback) to mouse-over and clicking can be added via a third language called **CSS**. This is the triad of main formal languages used in client side web development: HTML for contents, CSS for styling, and JavaScript for actions.
