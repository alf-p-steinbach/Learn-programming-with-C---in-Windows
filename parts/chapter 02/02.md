...
Third detail: the address that the browser presents is a little different than the one in chapter 1. That’s because in chapter 1 I used a *logical drive* `L:` to refer to the folder where the file was. Here instead the file is accessed directly via its full filesystem path.
