...
- [2.2 &nbsp;Static HTML &rarr; event &rarr; dynamic JavaScript execution.](#22-static-html-rarr-event-rarr-dynamic-javascript-execution)
...
You may rightly wonder what the difference is between using JavaScript to present a line of text, versus just writing that text line directly as HTML?
...
This difference is better observed with an example where the message box only pops up after the page content has been presented, e.g. when the user clicks a button. Events involving an HTML button, such as clicking it, can be *connected* to JavaScript by writing e.g. `onclick=`&hellip;, with the JavaScript code enclosed in HTML double quote characters. It is for this reason that single quote characters are used in the JavaScript code itself:
