...
Terminology: the JavaScript code that is connected to an event and executed when the event happens, is called an **event handler**. But often this code is very short, just specifying that a named chunk of code, defined somewhere else, should be executed. Then also that named chunk is called an event handler; it handles the event.
...
### 2.3 &nbsp;Dynamic JavaScript execution &rarr; changes to the HTML.

In addition to being connected to events, JavaScript code can inspect and change the HTML code.
