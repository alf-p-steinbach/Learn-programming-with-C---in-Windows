...
Another point: the file icon is Google Chrome’s because this user has installed Google Chrome and set it as his default browser.
...
The address that the browser presents is a little different than the one in chapter 1. That’s because in chapter 1 I used a *logical drive* `L:` to refer to the folder where the file was. Here instead the file is accessed directly via its full filesystem path.
