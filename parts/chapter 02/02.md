...
Immediately after an `=` assignment the value of the value holder thing on the left side is necessarily equal to the specified right hand side value.

But the next `=` assignment can change that, so it’s absolutely not equality forever, it’s a much more ephemeral effect!, and it’s absolutely not a standing order to maintain a correspondence or equality, just a value replacement that happens *here*, when the `=` executes.

Now we can for example use this DOM changing capability  &mdash; combining `this`, and dot-notation for object part access, and `=` assignment to replace values &mdash; in a program where you can click a button just once.
...
Practical tip: to **reload** the page so you can try again, e.g. after making changes to the source code, just hit `F5` or `Ctrl`+`R`.
...
&hellip; with two instructions (two semicolons!), is at best unconventional. One doesn’t usually write code like that. Instead one defines a named chunck of JavaScript, a **function**, like

```javascript
function handleButtonClick( eventInfo )
{
    eventInfo.target.innerText = 'Has been clicked.';
    eventInfo.target.disabled = true;
}
```

&hellip; and connects it to the event so that this code will be executed when the event happens.

And yes, this is about *defining new instructions*, as was mentioned in the first chapter.
...
The round parenthesis in the definition’s first line is a place to define names of **parameters**, information that is passed to the function by the invoking code. The above function defines one parameter called `eventInfo`, because when this function is executed as an event handler it receives such a parameter. With the way that we’ve so far used to connect events with JavaScript, which is a simple but old and somewhat outdated way, sniff sniff it has an odour!, the value that should go into the parameter must unfortunately be explicitly specified:

```html
<button onclick="handleButtonClick( window.event )">
```

Here `window` is a DOM object representing the tab window in the browser and its part `window.event` is the value that should go into the parameter, in this invocation of the function. It’s called an **argument** for the parameter. *This* round parenthesis with the argument is more generally an **argument list**, and that’s what says that the function should be executed, a function **call**. If there are no parameters then one must use an empty argument list, `()`; otherwise the function name would just refer to the function and the function would then not be called.

So, round parenthesis = the JavaScript, C# and C++ “call operator” when it follows a function name; it says that the function should be called, that you’re not for whatever reasons trying to just refer to that function.

---

In JavaScript it’s entirely OK that a function is used in a place in the source code before where it’s defined.

So, using and defining a function like above can go like this:

[*single-click-button-v2.js.html*](code/single-click-button-v2.js.html):

```html
This is the “Single click button!” program version 2.
...
The code uses `eventInfo.target` instead of just `this` for good reason: because in the execution of this function `this` no longer refers to the button object &mdash; but `eventInfo.target` does.

Example run, just the initial page display and effect of clicking the button:

<center>
<img src="images/single click button v2/sshot-1.png"><br>
↓<br>
<img src="images/single click button v2/sshot-2.png"><br>
</center>
