...
A **programming language** is a set of rules for how to define new instructions (you’ll do that all the time!), and how to use instructions: what you can and must write to use them. To write the pure text specification of a program you can use almost any text editor, including Windows’s own Notepad. But in order to create an executable program from that text, called a **compilation** of the text, you need tools that support the programming language that you’ve used, i.e. that support these rules; mainly a **compiler** for the language. Windows only provides these tools for Microsoft’s language C#. And so for other languages &mdash; including this book’s main language C++ &mdash; you need to download and install the requisite tools.

However, an alternative to creating an executable program is to get an existing executable program, such as your web browser, to let itself be directed by your textual instructions, called **interpretation**. I.e. your textual instructions are then instructions for that program, telling it what to do, with that program more or less directly “executing” your instructions. It’s only possible if the existing program does support being directed by textual instructions, but web browsers do; all web browsers have a built in JavaScript **interpreter**.
